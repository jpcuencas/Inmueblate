...
{
public void DesbloquearPeticionAmistad (int p_oid)
{
        /*PROTECTED REGION ID(NuevoInmueblateGenNHibernate.CEN.RedSocial_PeticionAmistad_desbloquearPeticionAmistad) ENABLED START*/

        PeticionAmistadCEN petCEN = new PeticionAmistadCEN ();

        PeticionAmistadEN peticion = petCEN.DamePeticionAmistadPorOID (p_oid);

        peticion.Estado = Enumerated.RedSocial.EstadoSolicitudAmistadEnum.pendiente;
        if (peticion.Estado != Enumerated.RedSocial.EstadoSolicitudAmistadEnum.pendiente)
