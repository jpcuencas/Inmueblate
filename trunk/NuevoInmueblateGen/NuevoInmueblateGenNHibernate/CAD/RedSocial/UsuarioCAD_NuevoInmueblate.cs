...
public System.Collections.Generic.IList<NuevoInmueblateGenNHibernate.EN.RedSocial.ElementoMultimediaEN>        GetAllFotosOfFusuario_NuevoInmueblate (int id)
{
        ...
        try
        {
                 ...
                 * IQuery query = session.CreateQuery(sql).SetParameter("p_Id", id);
                 */
                String sql = @"select self.Fotos FROM UsuarioEN self where self.Id=:p_Id";
                IQuery query = session.CreateQuery (sql).SetParameter ("p_Id", id);
...
public System.Collections.Generic.IList<NuevoInmueblateGenNHibernate.EN.RedSocial.ElementoMultimediaEN>        GetAllVideosOfVusuario_NuevoInmueblate (int id)
{
        System.Collections.Generic.IList<NuevoInmueblateGenNHibernate.EN.RedSocial.ElementoMultimediaEN> result = null;
        try
        {
                 ...
                 * IQuery query = session.CreateQuery(sql).SetParameter("p_Id", id);
                 */
                String sql = @"select self.Videos FROM UsuarioEN self where self.Id=:p_Id";
                IQuery query = session.CreateQuery (sql).SetParameter ("p_Id", id);
        ...
        try
        {
                using (ISession session = NHibernateHelper.OpenSession ())
                        using (ITransaction tx = session.BeginTransaction ())
                        {
                                 ...
                                 */
                                String sql = @"select count(self.Fotos) FROM UsuarioEN self where self.Id=:p_Id";
                                IQuery query = session.CreateQuery (sql).SetParameter ("p_Id", id);
                                result = query.UniqueResult<Int64>();
                        }
        }
...
public long GetAllVideosOfVusuarioCount_NuevoInmueblate (int id)
{
        long result = 0;
        ...
        try
        {
                using (ISession session = NHibernateHelper.OpenSession ())
                        using (ITransaction tx = session.BeginTransaction ())
                        {
                                 ...
                                 * String sql = @"select count(*) FROM ElementoMultimediaEN self inner join self.Usuario as target with target.Id=:p_Id";
                                 */
                                String sql = @"select count(self.Videos) FROM UsuarioEN self where self.Id=:p_Id";
                                IQuery query = session.CreateQuery (sql).SetParameter ("p_Id", id);
                                result = query.UniqueResult<Int64>();
                        }
        }
